...
def pintar_productos(request, id=0, pagina=0):
    if request.method == 'GET':
        if pagina:
            ...
            datos = {'message': "Success", 'productos': listado_pg, 'totalPaginas': int(total_pg)}
            # print(listado_pg)
            ...
            if request.data:
                if 'username' in request.data and 'password' in request.data:
                    try:
                        ...
                        user = User.objects.create_user(username=newUserUsername, password=newUserPassword)
                        user.is_staff = True
                        user.save()
...
def eliminar_usuario(request, id=0):
    if request.method == 'GET':
        ...
        if len(users) > 0:
            ...
            i = 0
            for user in users:
                listUsers.append({'username': user.username, 'id': user.id, 'is_superuser': user.is_superuser, 'last_login': user.last_login, 'date_joined': user.date_joined})
                # i += 1
...
def dashboard(request):
    if request.method == 'GET':
        ...
        for venta in numVentas:
            ...
            gananciasTotales += venta.precioTotal

        print('Las ganancias totales son: ' + str(gananciasTotales))

        data = {'message': "Success", 'numPedidos': numPedidos, 'numStock': numStock, 'gananciasTotales': gananciasTotales}
    else:
        return HttpResponseNotAllowed(['GET'])
...
def crear_producto(request):
    if request.method == 'POST':
        if request.POST and request.FILES:
            Producto.objects.create(nombre=request.POST['nombre'], descripcion=request.POST['descripcion'],categoria=request.POST['categoria'], colores=request.POST['colores'], estado=request.POST['estado'], precio=request.POST['precio'], stock=request.POST['stock'], imagen=request.FILES['img'])
            ...
            if len(jd) > 1:
                if jd['orden'] == 'Restar':
                    for producto in jd['productos']:
                        ...
                        actProducto.stock = actProducto.stock - producto['cantidad']
                        actProducto.save()
                elif jd['orden'] == 'Sumar':
                    for producto in jd['productos']:
                        actProducto = Producto.objects.get(id=producto['id'])
